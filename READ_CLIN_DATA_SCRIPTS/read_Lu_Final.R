...
dataPath <- "/volumes/cferte/NSCLC_Prognostic_Signature/Lu_GSE/publication/"
dataPath1 <- "/volumes/cferte/NSCLC_Prognostic_Signature/Lu_GSE/series_matrix/"



## READ IN THE TAB DELIMITED FILES
dat1 <- read.delim2(file = paste(dataPath, "Lu_Clin_Data.txt", sep=""), nrows=324, skip=2, header=T, na.strings=c("NA", "", " "), as.is=T,dec=",")
dat2  <- read.delim2(file = paste(dataPath1, "GSE6253-GPL96_series_matrix.txt", sep=""), nrows=2, skip=32, header=T, na.strings=c("NA", "", " "), as.is=T,dec=",")
dat3  <- read.delim2(file = paste(dataPath1, "GSE6253-GPL97_series_matrix.txt", sep=""), nrows=2, skip=32, header=T, na.strings=c("NA", "", " "), as.is=T,dec=",")
dat4  <- read.delim2(file = paste(dataPath1, "GSE6253-GPL8300_series_matrix.txt", sep=""), nrows=2, skip=32, header=T, na.strings=c("NA", "", " "), as.is=T,dec=",")


datN <- merge(x=dat2,y=dat3, all.x=T, all.y=T)
datN <- merge(x=datN,y=dat4, all.x=T, all.y=T)
datN <- t(datN)
datN <- as.data.frame(datN)
rm(dat4,dat2,dat3)

datN$CEL_ID <- datN[,1]
datN$SAMPLE_ID <- rownames(datN)
datN <- datN[,-2]
datN <- datN[,-1]
datN <- datN[-1,]

## remove the first 37 sample since they come from wrong platform
datN <- datN[-(1:37),]


table(dat1$Institute.Author)
...
datN <- merge(x=dat2,y=dat3,by="CEL_ID", all.x=T, all.y=T)

## these are the different columns we want to create
...
colnames(dat1)
GirardData  <- dat1[,c("CEL_ID", "Histology", "H_Grade","P_Stage","GENDER", "SMOKING", "RACE","totalAdjuvant", "VITAL_STATUS", "MONTHS_TO_LAST_CONTACT_OR_DEATH", "REC_STATUS" ,"TIME_TO_PROGRESSION_OR_LAST_CLIN_ASSESSMENT","LABORATORY_BATCH")]


## define CEL path in order to scan the date of production of the cel files
celPath  <- "/volumes/cferte/NSCLC_Prognostic_Signature/Girard_GSE31908/CEL_Files/GPL570_HGU133plus2/"
library(affy)
