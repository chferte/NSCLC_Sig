...
## supervised normalization of the probes according to the survival
#################################################################################################################
myCoxP <- function(x){
...
for(i in 1:dim(GRID)[1]){
  print(i)
  fit  <- try(glmnet(x,y,family="cox",maxit=1000000,alpha=GRID$.alpha[i],lambda=GRID$.lambda[i],penalty.factor=c(rep(1,293),0,0,0,0)))
...
#the top probes are stored in top
top<-rownames(fit$beta)[which(fit$beta!=0)]
top <- top[-101]



## find the gene symbols that are corresponding to the metaGEO summarized probes
require("mg.hgu133a.db")
sym <- as.character(mg.hgu133aSYMBOL)
topGenes <- sym[match(top,names(sym))]
