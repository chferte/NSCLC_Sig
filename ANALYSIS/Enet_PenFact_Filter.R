...
mySurv <-Surv(DirClinF$MONTHS_TO_LAST_CONTACT_OR_DEATH,DirClinF$VITAL_STATUS)
P_TS <- apply(MATRIX_TS, 1, myCoxP)
P_TS <- p.adjust(P_TS, method = "BH", n = length(P_TS))
P_TS1 <- apply(MATRIX_TS1, 1, myCoxP)
P_TS1 <- p.adjust(P_TS1, method = "BH", n = length(P_TS1))
table(P_TS<.05)

mySurv <-Surv(ZhuClinF$MONTHS_TO_LAST_CONTACT_OR_DEATH,ZhuClinF$VITAL_STATUS)
P_VS <- apply(MATRIX_VS, 1, myCoxP)
table(P_VS<.05)
P_VS <- p.adjust(P_VS, method = "BH", n = length(P_VS))
P_VS1 <- apply(MATRIX_VS1, 1, myCoxP)
table(P_VS1<.05)
P_VS1 <- p.adjust(P_VS1, method = "BH", n = length(P_VS1))
table(P_VS<.05)
table(P_VS1<.05)
...
tmp <- which(P_TS<.1)
length(tmp)
MATRIX_TS <- MATRIX_TS[tmp,]
MATRIX_TS1 <- MATRIX_TS1[tmp,]
MATRIX_VS <- MATRIX_VS[tmp,]
MATRIX_VS1 <- MATRIX_VS1[tmp,]
...
tmp <- DirClinF[,c("P_Stage","Histology")]
...
tmp1 <- ZhuClinF[,c("P_Stage","Histology")]
...
for(i in 1:dim(GRID)[1]){
  i
  fit  <- try(glmnet(x,y,family="cox",maxit=1000000,alpha=GRID$.alpha[i],lambda=GRID$.lambda[i],penalty.factor=c(rep(1,293),0,1)))
...
rownames(CI_TOTAL)<- c(1:(dim(GRID)[1]))
...
## build the Grid alpha/lambda
alphas <- seq(.5,.95,.05)
lambdas <-  c(10^(seq(0,10,.5)))
GRID <- expand.grid(.family="cox",.alpha=alphas,.lambda=lambdas)

CI_TS <- c()
CI_VS <- c()
for(i in 1:dim(GRID)[1]){
  fit  <- try(glmnet(x,y,family="cox",alpha=GRID$.alpha[i],lambda=GRID$.lambda[i],maxit=1000000,penalty.factor=c(rep(1,12161),rep(0,4))))
